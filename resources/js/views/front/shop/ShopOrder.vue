<script setup>
    ...
    const phoneRules = [(v) => !!v || '請輸入聯絡電話', (v) => /^09\d{8}$/.test(v) || '請輸入正確的手機格式']
    const addressRules = [(v) => !!v || '請輸入收件地址']
    const billRules = [(v) => !!v || '請選擇發票類型']

    const taxIdRules = computed(() => (form.value.bill === '公司行號' ? [(v) => !!v || '請輸入統一編號', (v) => /^\d{8}$/.test(v) || '統編格式應為 8 位數字'] : []))
    const carrierRules = computed(() =>
        form.value.bill === '手機載具' ? [(v) => !!v || '請輸入載具號碼 (例: /ABC1234)', (v) => /^\/[A-Z0-9.+-]{7}$/.test(v) || '載具格式錯誤 (例: /ABC1234)'] : []
    )
    const cardNumberRules = computed(() =>
        form.value.paymentMethod === 'Credit card' ? [(v) => !!v || '請輸入信用卡號', (v) => (v && v.length === 16) || '卡號應為 16 位數'] : []
    )
    const cvcRules = computed(() => (form.value.paymentMethod === 'Credit card' ? [(v) => !!v || '請輸入安全碼', (v) => (v && v.length === 3) || '應為 3 位數'] : []))
    const dateRules = computed(() => (form.value.paymentMethod === 'Credit card' ? [(v) => !!v || '請輸入卡片有效日期'] : []))
        ...
        api.post('/orders', payload)
            .then((res) => {
                ...
                if (res.data.message === 'ECPay' && res.data.form_html) {
                    ...
                    const form = div.querySelector('#ecpay_form')
            ...
            .catch((error) => {
                clearSensitiveFields()
                notify(error.response?.data?.message || '訂單提交失敗，請稍後再試', 'error')
            })
            .finally(() => {
...
<template>
    <FrontLayout>
        <v-container style="max-width: 1200px" class="py-6 px-4">
            ...
            <v-btn variant="outlined" color="primary" class="mb-6" prepend-icon="mdi-arrow-left" @click="goTo('/cart')">返回購物車</v-btn>

            <v-form ref="checkoutFormRef">
                <v-row>
                    <v-col cols="12" md="8">
                        <v-card rounded="xl" elevation="2" class="mb-5">
                            <div class="text-subtitle-1 font-weight-medium d-flex align-center ga-2 pt-4 pb-1 px-4">
                                ...
                                <v-table density="compact">
                                    ...
                                    <tbody>
                                        <tr v-for="item in cartItems" :key="item.id">
                                            <td>
                                                <v-img :src="getImageUrl(item.image)" width="80" height="60" cover rounded="sm" />
                                            </td>
                                            <td>{{ item.name }}</td>
                                            <td>{{ item.quantity }}</td>
                                            <td>NT$ {{ item.price }}</td>
                        ...
                        <v-card rounded="xl" elevation="2" class="mb-5">
                            <div class="text-subtitle-1 font-weight-medium d-flex align-center ga-2 pt-4 pb-1 px-4">
                                ...
                                購買人訊息
                                <v-checkbox v-model="userInfo" color="info" label="與上次購買資料同" hide-details @update:model-value="fillFromLastOrder"></v-checkbox>
                            </div>
                            <v-card-text>
                                <v-text-field
                                    ref="nameRef"
                                ...
                                <v-text-field
                                    ...
                                    :rules="phoneRules"
                                    class="mb-2"
                                    @keydown.enter.prevent="focusNext(addressRef)"
                                />
                                <v-text-field
                                    ref="addressRef"
                        ...
                        <v-card rounded="xl" elevation="2" class="mb-5">
                            ...
                            <v-card-text>
                                <v-tabs v-model="form.paymentMethod" color="primary" density="compact" class="mb-4" @update:model-value="handleTabChange">
                                ...
                                <v-tabs-window v-model="form.paymentMethod">
                                    <v-tabs-window-item value="Credit card">
                                        <v-text-field
                                            ...
                                            class="my-2"
                                            @keydown.enter.prevent="focusNext(dateRef)"
                                        />
                                        <v-row>
                                            <v-col cols="6">
                                                <v-text-field
                                                    ...
                                                    type="month"
                                                    :rules="dateRules"
                                                    @keydown.enter.prevent="focusNext(cvcRef)"
                                                />
                                            </v-col>
                                            <v-col cols="6">
                                                <v-text-field
                        ...
                        <v-card rounded="xl" elevation="3" class="sticky-card">
                            ...
                            <v-card-text>
                                ...
                                <div class="d-flex justify-space-between text-body-2">
                                    <b>應付總額</b>
                                    <b class="text-error">NT$ {{ totalPrice + SHIPPING_FEE }}</b>
                                </div>
                            </v-card-text>
                            <v-card-actions class="px-4 pb-4">
                                <v-btn block variant="outlined" color="error" size="large" rounded="lg" :loading="isSubmitting" @click="submitOrder">確認下單</v-btn>
                            </v-card-actions>
                        </v-card>
                    </v-col>
                </v-row>
