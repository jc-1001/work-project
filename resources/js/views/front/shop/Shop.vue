<script setup>
    ...
    const fetchProducts = () => {
        ...
        if (typeof selectedCategoryId.value === 'number') params.category_id = selectedCategoryId.value
...
<template>
    <FrontLayout>
        <div class="text-white text-center pt-12 pb-10 px-6 mb-2" style="background-color: #0f3460">
            <h1 class="font-weight-bold mb-2" style="font-size: 2rem; letter-spacing: 2px">商城</h1>
            <p class="mb-0" style="font-size: 0.95rem; opacity: 0.7">共 {{ total }} 件商品</p>
        </div>
        ...
        <template v-else>
            <div v-if="products.length === 0" class="d-flex flex-column align-center py-15">
            ...
            <v-row v-else class="px-4 py-2">
                <v-col v-for="product in products" :key="product.id" cols="12" sm="6" md="4" lg="3" class="mb-1">
                    <v-card class="product-card h-100 cursor-pointer" @click="goToProductDetail(product.id)">
                        <div class="position-relative">
                            <v-img :src="getImageUrl(product.image)" height="200" cover />
                            <v-chip v-if="product.stock === 0" color="error" style="position: absolute; top: 8px; left: 8px">已售完</v-chip>
                        </div>
                        <v-card-text class="pb-1">
                            <div class="text-body-2 mb-1 text-truncate text-grey-darken-3">
                                {{ product.name }}
                            </div>
                            <div class="font-weight-bold text-primary">NT$ {{ Number(product.price).toLocaleString() }}</div>
                        </v-card-text>
