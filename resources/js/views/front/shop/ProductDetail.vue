<script setup>
    ...
    const allImages = computed(() => {
        ...
        ;(product.value.product_images ?? []).forEach((img) => imgs.push(img.image_path))
        return imgs
    })

    const activeImageUrl = computed(() => getImageUrl(allImages.value[activeImageIndex.value] ?? ''))
    ...
    <FrontLayout>
        <div class="mx-auto px-6 pt-6 pb-16" style="max-width: 1100px">
            ...
            <v-row v-else align="start" no-gutters class="ga-14">
                <v-col cols="12" sm>
                    ...
                    <div v-if="allImages.length > 1" class="d-flex ga-2 mt-3 flex-wrap">
                        <div
                            ...
                            @mouseenter="thumbHoveredIndex = i"
                            @mouseleave="thumbHoveredIndex = null"
                            @click="activeImageIndex = i"
                        >
                            <v-img :src="getImageUrl(img)" cover width="56" height="56" />
                        </div>
                    </div>
                </v-col>
                ...
                <v-col cols="12" sm>
                    <div class="d-flex flex-column ga-5">
                        ...
                        <div class="rounded-e-lg" style="border-left: 4px solid rgb(var(--v-theme-primary)); background: #f5f9ff; padding: 14px 18px">
                            <p class="ma-0 text-medium-emphasis" style="font-size: 0.95rem; line-height: 1.8">
                                {{ product.description }}
                            </p>
                        </div>
        ...
        <v-dialog v-model="imageDialog" max-width="800">
            <v-card color="black" flat class="position-relative">
                ...
                <v-img :src="activeImageUrl" contain max-height="80vh" />
                ...
                <template v-if="allImages.length > 1">
                    <v-btn
                        ...
                        color="white"
                        variant="flat"
                        style="position: absolute; left: 18px; top: 50%; transform: translateY(-50%); z-index: 2; opacity: 0.85"
                    ...
                    <v-btn
                        ...
                        color="white"
                        variant="flat"
                        style="position: absolute; right: 18px; top: 50%; transform: translateY(-50%); z-index: 2; opacity: 0.85"
